  ...
  <div v-if="show" class="modal-overlay" @click="$emit('close')">
    <div class="modal-content players-modal" @click.stop>
      ...
      <div class="modal-body">
        <div v-if="server && teamGroups.length" class="leaderboard-section">
          <!-- Mobile Team Tabs -->
...
const teamGroups = computed(() => {
  if (!props.server || !props.server.players.length) return [];
  
  // First, sort players by score (descending) and assign ranks
  const sortedPlayers = [...props.server.players].sort((a, b) => b.score - a.score);
  const playersWithRank = sortedPlayers.map((player, index) => ({
    ...player,
    rank: index + 1,
    // Handle empty teamLabel by looking it up in teams array
    teamLabel: player.teamLabel || props.server!.teams.find(t => t.index === player.team)?.label || 'Unknown'
  }));
