...
import { fetchRoundsList, RoundPagedResult, RoundListItem, ServerContextInfo } from '../services/roundsService';
...
const serverContext = ref<ServerContextInfo | null>(null);
...
const fetchData = async () => {
  ...
  try {
    ...
    // Fetch the rounds with pagination, filters, and sorting
    const result = await fetchRoundsList(
      currentPage.value, 
      pageSize.value,
      sortBy.value,
      sortOrder.value,
    ...
    serverContext.value = result.serverContext || null;
    ...
    error.value = 'Failed to fetch rounds. Please try again.';
  } finally {
    loading.value = false;
  }
};
...
const navigateToRoundReport = (round: RoundListItem, event?: Event) => {
...
const getTimeGap = (currentRound: RoundListItem, nextRound: RoundListItem): string => {
  ...
  const next = new Date(nextRound.startTime.endsWith('Z') ? nextRound.startTime : nextRound.startTime + 'Z');
  ...
  <div class="player-sessions-page-container">
    <div class="header">
      <div class="header-left">
        <h1>Game Rounds</h1>
        <div v-if="serverContext" class="player-info">
          <div class="info-item">
    ...
    <div v-if="loading && rounds.length === 0" class="loading">Loading rounds data...</div>
    <div v-else-if="error" class="error">{{ error }}</div>
    <div v-else-if="rounds.length > 0" class="sessions-table-container">
      <!-- Rounds count and pagination info -->
      <div class="table-header">
        <div class="sessions-count">
          Showing {{ (currentPage - 1) * pageSize + 1 }} - {{ Math.min(currentPage * pageSize, totalItems) }} of {{ totalItems }} rounds
        </div>
        <div class="page-size-selector">
          <label for="pageSize">Rounds per page:</label>
      ...
      <div class="timeline-container">
        <template v-for="(round, index) in rounds" :key="round.roundId">
          ...
          <div class="timeline-item">
            ...
            <div 
              class="session-card"
              @click="(event) => navigateToRoundReport(round, event)"
            >
              ...
              <div class="session-line-2">
                ...
                <span class="game-type">({{ round.gameType }})</span>
                <span v-if="round.isActive" class="badge-active">Live</span>
              </div>
    ...
    <div v-else class="no-data">No rounds found.</div>
  </div>
</template>
