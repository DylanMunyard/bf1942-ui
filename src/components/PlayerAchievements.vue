...
const gamificationData = ref<PlayerAchievementSummary | null>(null);
...
const _getTierGlow = (tier: string): string => {
...
const _getTimeGap = (currentStreak: Streak, nextStreak: Streak): string => {
...
const _navigateToRoundReport = (streak: Streak) => {
  router.push({
    name: 'round-report',
    params: {
...
<template>
  <div class="relative">
    ...
    >
      <div class="w-12 h-12 border-4 border-slate-600/30 border-t-cyan-400 rounded-full animate-spin mb-4" />
      <p class="text-lg font-medium">
        Loading achievements...
      </p>
      <p class="text-sm text-slate-500 mt-2">
        Scanning battlefield records
      </p>
    </div>
    ...
    <div
      v-if="gamificationData"
      class="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 mb-6"
    >
      <!-- Trophy Icon -->
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div class="bg-slate-800/50 rounded-lg p-3 border border-slate-700/50">
            <div class="text-lg font-bold text-orange-400">
              {{ gamificationData.bestStreaks.bestSingleRoundStreak || 0 }}
            </div>
            <div class="text-xs text-slate-400">
              Best Streak
            </div>
    ...
    <div
      v-if="gamificationData && (gamificationData.bestStreaks.bestSingleRoundStreak > 0 || nextMilestone)"
      class="mb-6"
    >
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
          <!-- Animated background -->
          <div class="absolute inset-0 bg-gradient-to-br from-yellow-500/10 via-orange-500/5 to-red-500/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
          ...
          <div class="relative z-10 p-6">
            <div class="flex flex-col items-center text-center space-y-4">
              <div class="relative">
                ...
                >
                <div class="absolute inset-0 bg-gradient-to-t from-yellow-500/20 to-transparent rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
              </div>
              ...
              <div class="space-y-2">
                ...
                <div class="text-2xl font-extrabold text-white">
                  {{ gamificationData.bestStreaks.bestSingleRoundStreak }}
                </div>
          ...
          <div class="relative z-10 p-6">
            <div class="flex flex-col items-center text-center space-y-4">
              <div class="relative">
                ...
                >
                <div class="absolute inset-0 bg-gradient-to-t from-blue-500/20 to-transparent rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
              </div>
              ...
              <div class="space-y-2">
                ...
                <div class="text-2xl font-extrabold text-white">
                  {{ nextMilestone.milestone.toLocaleString() }}
                </div>
                <p class="text-sm text-slate-400">
                  {{ Math.floor(nextMilestone.progress * 100) }}% Complete
                </p>
                <p class="text-xs text-slate-500">
                  {{ nextMilestone.killsRemaining.toLocaleString() }} kills remaining
                </p>
              </div>
            </div>
          </div>
            ...
            <div class="flex flex-col items-center text-center space-y-4">
              <div class="relative">
                <img 
                  ...
                  class="w-24 h-32 object-contain transition-transform duration-300 group-hover:scale-110 drop-shadow-lg"
                >
                <div class="absolute inset-0 bg-gradient-to-t from-emerald-500/20 to-transparent rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
              </div>
              ...
              <div class="space-y-2">
                ...
                <div class="text-lg font-bold text-white">
                  {{ gamificationData.recentAchievements[0].achievementName }}
                </div>
                <p class="text-sm text-slate-400 capitalize">
                  {{ gamificationData.recentAchievements[0].tier }} Tier
                </p>
                <p class="text-xs text-slate-500">
                  {{ formatRelativeTime(gamificationData.recentAchievements[0].achievedAt) }}
                </p>
              </div>
            </div>
          </div>
    ...
    <div
      v-if="gamificationData && combinedStreaks.length > 0"
      class="mb-6"
    >
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-4">
        ...
        >
          <!-- Streak glow effect -->
          <div class="absolute inset-0 bg-gradient-to-br from-orange-500/10 to-red-500/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      ...
      <div
        v-if="combinedStreaks.length > 16"
        class="text-center"
      >
    ...
    <div
      v-if="gamificationData && flattenedAchievements.length > 0"
      class="mb-6"
    >
      ...
      <div
        v-if="flattenedAchievements.length > 36"
        class="text-center"
      >
        <router-link
    ...
    >
      <div class="relative mb-8">
        <div class="text-8xl opacity-20 animate-pulse">
          🏆
        </div>
        <div class="absolute inset-0 flex items-center justify-center">
          <div class="w-32 h-32 bg-gradient-to-r from-purple-500/10 to-blue-500/10 rounded-full blur-xl" />
        </div>
      </div>
    ...
    <Teleport to="body">
      ...
      >
        ...
        >
          ...
          <div
            class="absolute bottom-0 left-0 h-1.5 bg-gradient-to-r from-cyan-400 to-yellow-400 transition-all duration-700 z-10"
            :style="{ width: nextMilestone.progress * 100 + '%' }"
          />
