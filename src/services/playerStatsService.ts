...
/**
 * Fetches round report for a specific session
 * @param sessionId The ID of the session
 * @returns Round report with leaderboard snapshots
 */
export async function fetchRoundReport(sessionId: number): Promise<RoundReport> {
  try {
    // Make the request to the API endpoint
    const response = await axios.get<RoundReport>(`/stats/players/sessions/${sessionId}/round-report`);
