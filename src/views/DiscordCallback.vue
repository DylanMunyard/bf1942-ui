<template>
  <div class="flex items-center justify-center min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
    <div class="text-center">
      <div v-if="error" class="bg-red-500/10 border border-red-500/50 rounded-xl p-6 max-w-md">
        ...
        <button
          ...
          class="mt-4 px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg transition-colors"
        >
          Return Home
        </button>
      </div>
      <div v-else class="bg-slate-800/60 backdrop-blur-sm border border-slate-700/50 rounded-xl p-6">
        ...
        <h2 class="text-xl font-bold text-slate-200">Signing in with Discord...</h2>
        <p class="text-slate-400 mt-2">Please wait while we complete your authentication</p>
      </div>
    </div>
  </div>
</template>
